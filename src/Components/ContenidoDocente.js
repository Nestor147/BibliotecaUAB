...
import AdminDocenteMisEstudiantes from './DocenteMisEstudiantes';
...
export default function ContenidoDocente(props) {
    ...
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
          <Tab label="Mis Estudiantes" {...a11yProps(0)} />
          <Tab label="Mis Materias" {...a11yProps(1)} />
        </Tabs>
      </Box>
      <TabPanel value={value} index={0}>
        <DocenteMisEst user={props.user} />
      </TabPanel>
      <TabPanel value={value} index={1}>
        <DocenteMisMaterias user={props.user}/>
      </TabPanel>
    </Box>
  );
}
